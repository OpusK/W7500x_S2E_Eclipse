...
#if defined(ENABLE_CFGTIME)
	printf("[DB] currentTime1 %lu\r\n", s2e_packet->options.currentTime);
	tmpTime = s2e_packet->options.currentTime + 2208988800UL;
	printf("[DB] currentTime2 %lu %lu\r\n", tmpTime, g_seconds);
	currentTime[3] = (uint8_t) tmpTime;
	currentTime[2] = (uint8_t) (tmpTime >> 8);
	currentTime[1] = (uint8_t) (tmpTime >> 16);
	currentTime[0] = (uint8_t) (tmpTime >> 24);
	get_seconds_from_ntp_server(currentTime,0);
    getTime(&time);
	printf("[DB] Local Time %d/%d/%d %d:%d:%d %lu\r\n", time.yy, time.mo, time.dd, time.hh, time.mm, time.ss, g_seconds);
	s2e_packet->options.currentTime = g_seconds;
    //calcdatetime(g_seconds);
    //getTime(&time);
	//printf("[DB] Time %d/%d/%d %d:%d:%d %lu\r\n", time.yy, time.mo, time.dd, time.hh, time.mm, time.ss, g_seconds);
#endif
