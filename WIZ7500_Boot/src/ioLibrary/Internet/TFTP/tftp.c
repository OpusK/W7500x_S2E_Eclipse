...
uint32_t g_server_ip_check = 0;
uint16_t g_server_port_check = 0;
...
uint8_t g_tftp_rcv_buf[MAX_MTU_SIZE];
...
void TFTP_init(uint8_t socket, uint8_t *buf)
{
	...
	//g_tftp_rcv_buf = buf;
	memset(&g_tftp_rcv_buf[0], 0x00, MAX_MTU_SIZE);
}
...
void TFTP_exit(void)
{
	...
	//g_tftp_rcv_buf = NULL;
	memcpy(&g_tftp_rcv_buf[0], 0x00, MAX_MTU_SIZE);
}
